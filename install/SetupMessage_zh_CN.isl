...
SetupAlreadyRunning=安装向导已经在运行中。
WindowsVersionNotSupported=此程序不支持你的计算机运行的 Windows 版本。
WindowsServicePackRequired=此程序要求 %1 Service Pack %2 或更高版本。
...
SelectDirBrowseLabel=若要继续，单击 [下一步]。如果你要选择不同的文件夹，请单击 [浏览]。若您选择安装在系统文件夹，例如：「C:\Program Files」，请按「取消」，并重新以管理员身份运行 setup.exe 软件。
DiskSpaceMBLabel=至少需要 [mb] MB 的空闲磁盘空间。
CannotInstallToNetworkDrive=安装向导不能安装到网络驱动器。
CannotInstallToUNCPath=安装向导不能安装到 UNC 路径。
...
ApplicationsFound=下列应用程序正在使用需要通过安装程序更新的文件。建议您允许安装程序自动关闭这些应用程序。
ApplicationsFound2=下列应用程序正在使用需要通过安装程序更新的文件。建议您允许安装程序自动关闭这些应用程序。在安装完成后，安装程序将尝试重新启动应用程序。
CloseApplications=自动关闭应用程序(&A)
DontCloseApplications=不关闭应用程序(&D)
ErrorCloseApplications=安装向导无法自动关闭所有的应用程序。建议您关闭所有应用程序有使用但却需要更新的文件，安装程序才能继续。
...
StatusClosingApplications=关闭应用程序...
...
AddonHostProgramNotFound=您选定的文件夹中找不到 %1。%n%n 你想继续吗？
StartFilelugDesktop=启动 Filelug Desktop
UninstallFilelugDesktop=卸载 Filelug Desktop
